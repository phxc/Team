	...
	<div id="page-wrapper">
		...
		<div id="header">
			...
			<div class="inner">
				<header>
					<h1>
						<a href="/board/list" id="logo">게시판</a>
					</h1>
				</header>
			</div>
			...
			<div class="row gtr-200">
				...
				<div class="col-8 col-12-mobile" id="content">
					...
					<article id="main">
						<header>
							<h2 align="center">
								<a href="#">제주 여행지 공유 공간</a>
							</h2>
							<c:choose>
								<c:when test="${sessionScope.user_email == null }">
									<button type="button"
										class="btn btn-primary btn-sm float-right col-1"
										style="float: right; margin-right: 20px;"
										onclick="regiCheck()">등록</button>
								</c:when>
								<c:otherwise>
									<button type="button"
										class="btn btn-primary btn-sm float-right col-1"
										style="float: right; margin-right: 20px;"
										onclick="location.href='register?pageNum=${pageMaker.cri.pageNum}&count=${pageMaker.cri.count }'">등록</button>
								</c:otherwise>
							</c:choose>
							<br> <br>
							<div class="card-body">
								<div class="table-responsive">
										...
										<c:forEach var="article" items="${board_list }">
											<tbody>
												<tr>
													...
													<td>
														<!-- 7.조회 페이지 이동 --> <a class="move"
														href='<c:out value="${article.bno}"/>'> <c:out
											...
											<form id='searchForm' action="list" method='get'>
												<div class="form-group">
													<select name='type' class="col-2"
														...
														<option style="text-align-last: center" value=""
															<c:out value="${pageMaker.cri.type == null?'selected':''}"/>>선택</option>
														<option style="text-align-last: center" value="T"
															<c:out value="${pageMaker.cri.type eq 'T'?'selected':''}"/>>제목</option>
														<option style="text-align-last: center" value="C"
															<c:out value="${pageMaker.cri.type eq 'C'?'selected':''}"/>>내용</option>
														<option style="text-align-last: center" value="W"
															<c:out value="${pageMaker.cri.type eq 'W'?'selected':''}"/>>작성자</option>
													</select> <input type="text" class="col-8"
									...
									<div
										class="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css">
										<div class="container">
											<div class="row">
												<div class="col">
													<ul class="pagination justify-content-center"
														...
														<c:if test="${pageMaker.prev }">
															<li class="page-item"><a class="page-link"
																href="list?pageNum=${pageMaker.startPage -1 }">Previous</a></li>
														</c:if>
														<!-- 2. 페이지번호 활성화 여부 -->
														...
														<c:if test="${pageMaker.next }">
															<li class="page-item"><a class="page-link"
																href="list?pageNum=${pageMaker.endPage +1 }">Next</a></li>
														</c:if>
													</ul>
												</div>
											</div>
					...
					<c:forEach var="articles" items="${top_list }">
						...
						<section>
							<div class="row gtr-100">
								<div class="col-4">
									<a href="#" class="image fit" style="margin-bottom: 50px;"><img src="${pageContext.request.contextPath}/images/top5.jpg" alt="" /></a>
								</div>
								<div class="col-8">
